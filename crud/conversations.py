...
from core.config import ecommerce_database_name, conversations_collection_name
from crud.messages import remove_messages_by_conversation_id_impl, \
...
async def add_conversation_impl(conversation_in: ConversationIn, conn: AsyncIOMotorClient) -> ConversationOut:
    row = await conn[ecommerce_database_name][conversations_collection_name].insert_one(conversation_in.dict())
...
async def get_conversation_by_id_impl(conversation_id: str, conn: AsyncIOMotorClient) -> ConversationOut:
    ...
    row = await conn[ecommerce_database_name][conversations_collection_name].find_one(query)
    if row:
...
async def remove_conversation_by_conversation_id_impl(conn: AsyncIOMotorClient, conversation_id: str):
    query = {"_id": ObjectId(conversation_id)}
    await conn[ecommerce_database_name][conversations_collection_name].delete_one(query)
...
async def update_complete_conversation_by_id(conversation_id: str, conversation_in: ConversationIn,
    ...
    await conn[ecommerce_database_name][conversations_collection_name].update_one({"_id": ObjectId(conversation_id)},
                                                                                  query)
