...
from core.config import ecommerce_database_name, messages_collection_name
...
async def send_message_impl(message_in: MessageIn, conn: AsyncIOMotorClient) -> MessageOut:
    row = await conn[ecommerce_database_name][messages_collection_name].insert_one(message_in.dict())
    mqtt_client.publish(topic=get_user_topic(account_id=message_in.user_owner_id),payload=json.dumps(message_in.dict()),qos=2)
...
async def get_message_by_id_impl(message_id: str, conn: AsyncIOMotorClient) -> MessageOut:
    ...
    row = await conn[ecommerce_database_name][messages_collection_name].find_one(query)
    if row:
...
async def remove_message_by_id_impl(conn: AsyncIOMotorClient, message_id: str):
    query = {"_id": ObjectId(message_id)}
    await conn[ecommerce_database_name][messages_collection_name].delete_one(query)


async def remove_messages_by_conversation_id_impl(conn: AsyncIOMotorClient, conversation_id: str):
    query = {"conversation_id": conversation_id}
    await conn[ecommerce_database_name][messages_collection_name].delete_many(query)
